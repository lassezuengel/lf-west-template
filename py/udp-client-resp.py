...
PEER_PORT = 4242
RECV_BUF_SIZE = 128
SEND_INTERVAL_MS = 500  # milliseconds between sends
...
class UDPClient:
    ...
    def send_udp_data(self):
        ...
        if ret < 0:
            ...
            logger.info(f"{self.proto} UDP: Sent {LOREM_IPSUM} to server")

        # Tries to receive response from server
        self.udp_sock.settimeout(5)
        try:
            data, svr_addr = self.udp_sock.recvfrom(RECV_BUF_SIZE)
            if not data:
                logger.info("Server disconnected")
            else:
                logger.info(f"Received {len(data)} bytes from server {svr_addr}: {data.decode('utf-8', errors='replace')}")
        except socket.timeout:
            logger.error("Server didn't answer, trying again soon...")
